...
def build_morning_message(tasks):
    ...
    lines = ["🦇 *SPECIAL ONE — Morning Brief*", f"📅 {today}", "",
    ...
    lines += ["", "💬 שלח טקסט להוספת משימה", "/tasks · /done · /stats"]
    return "\n".join(lines)
...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"*הוסף משימה:*\n`משימה` · `! משימה` High · `~ משימה` Low\n`משימה | 2026-03-15` עם דדליין",
        parse_mode="Markdown")
...
async def cmd_done(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not opened:
        await update.message.reply_text("אין משימות פתוחות!"); return
    buttons = [[InlineKeyboardButton(f"{pri_icon(t.get('pri','low'))} {t['name'][:38]}",
...
async def cb_done(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if write_tasks(tasks):
        ...
        await q.edit_message_text("❌ שגיאה.")
...
async def handle_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    deadline = ""
    if "|" in name:
        parts = name.split("|", 1); name, deadline = parts[0].strip(), parts[1].strip()
    tasks = read_tasks()
    tasks.append({"id": next_id(tasks), "name": name, "detail": "", "cat": "ARMA GIDEON",
                  "dept": "—", "pri": pri, "status": "open", "deadline": deadline,
                  "notes": "נוסף מהבוט", "checklist": [],
                  "created_at": datetime.now().strftime("%Y-%m-%d %H:%M")})
    if write_tasks(tasks):
        dl_str = f"\n📅 {deadline}" if deadline else ""
        await update.message.reply_text(f"✅ נוסף: {pri_icon(pri)} *{clean(name)}*{dl_str}", parse_mode="Markdown")
    else:
...
async def morning_brief(ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await ctx.bot.send_message(chat_id=int(ALLOWED_ID),
            text=build_morning_message(read_tasks()), parse_mode="Markdown")
    except Exception as e:
...
def main():
    ...
    logger.info(f"🦇 Running — brief at {MORNING_HOUR:02d}:{MORNING_MIN:02d}")
